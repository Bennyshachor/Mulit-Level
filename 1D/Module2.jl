...
function NewtonMethod(initial_x,Elem,Nodes,source,C,d,dAdk,L,alphaReg,tau,MaxIter)

    k_step = initial_x
    for iter in 1:MaxIter

        k    = k_step
        Cost = Objective(k,Nodes,Elem,source,L,alphaReg,C,d)
        grad = Grad(k,Elem,Nodes,source,C,d,dAdk,L,alphaReg)
        Hess = Hessian(k,Elem,Nodes,source,C,d,dAdk,L,alphaReg,tau)

        println("iter =", iter )
        println("Cost = ", Cost)
        println("Grad = ", norm(grad))

        dir     = -Hess\grad # the direction of progress
        r       = dir
        ...
        k_step  = k + 1*dir #didn't use linesearch for distance

            #computes the sign of the newton direction
        if iter == MaxIter
            return k_step
        end
        #    println(objective("objective = ", k_step,Nodes,Elem,source_loc,DirNodes,L,alphaReg,C,d))
        if norm(dir) <= 1e-5 || norm(grad)<=1e-8
            #println("norm k_step-k = " ,norm(k_step-k))
            #println("norm of grad = " ,norm(grad))
            #println("stop please")
            return k_step
        end
    end
