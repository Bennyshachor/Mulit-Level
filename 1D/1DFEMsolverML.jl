...
function CreateParameters()
    ...
    for i in 1:length(source)
        global u[:,i],~,~,~        = FEMmodel1D(FineNodes,FineElem,k_exact,source[i])
    ...
    for i = 1:length(FineElem)
        ei            = zeros(length(FineElem))
        ei[i]         = 1
        dAdk[:,:,i]  = FEMmodel1Ddadk(FineNodes,FineElem,ei,0.5) #source has no affect
    end


    return source, obs, alphaReg, tau, Nnodes, FineNodes, FineElem, k_exact, C, d, dAdk
end

source, obs, alphaReg, tau, Nnodes, FineNodes,
 FineElem, k_exact, C, d, dAdk = CreateParameters()
...
writedlm("C:/Users/benis/OneDrive/delft/julia/MATLAB/designML.txt",X)
writedlm("C:/Users/benis/OneDrive/delft/julia/MATLAB/deltaML.txt",delta)
writedlm("C:/Users/benis/OneDrive/delft/julia/MATLAB/gradML.txt",grad)
writedlm("C:/Users/benis/OneDrive/delft/julia/MATLAB/costML.txt",cost)
